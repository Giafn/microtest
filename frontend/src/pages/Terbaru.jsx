...
const Terpopular = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const response = await fetch(import.meta.env.VITE_BACKEND_URL + "/api/items?take=100&filterBy=date_desc"); // Endpoint untuk produk populer
          ...
          <div className="grid grid-cols-1 sm:grid-cols-8 lg:grid-cols-4 gap-8 mb-10 ml-8">
            {sortedProductData.map((product, index) => (
              <Card
                key={index}
                id={product.id}
                img={import.meta.env.VITE_BACKEND_URL + "/" + product.images[0].url}
                alt={product.name}
                title={product.name}
