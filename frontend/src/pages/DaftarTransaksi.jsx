      ...
      try {
        ...
        if (response.data.status === "success") {
          const apiTransactions = response.data.data.map((transaction) => ({
            ...
            image: import.meta.env.VITE_BACKEND_URL + "/" + transaction.items[0]?.images[0],
  ...
  const handleCheckPaymentStatus = async () => {
    try {
      ...
      const response = await axios.get(
        import.meta.env.VITE_BACKEND_URL + `/api/transactions/check/${selectedTransaction.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleUpdateTransactionStatus = async (transactionId, status) => {
    try {
      ...
      const response = await axios.post(
        import.meta.env.VITE_BACKEND_URL + `/api/transactions/status-update/${transactionId}`,
        { status },
        {
          headers: {
            Authorization: `Bearer ${token}`,
