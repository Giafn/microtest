...
const Pembayaran = () => {
  ...
  const handleStartPayment = async () => {
    try {
      ...
      const response = await axios.post(import.meta.env.VITE_BACKEND_URL + "/api/transactions/", requestData, {
        headers: {
  ...
  const handleCheckStatus = async () => {
    if (transactionId) {
      try {
        const token = localStorage.getItem("token");
        const response = await axios.get(import.meta.env.VITE_BACKEND_URL + `/api/transactions/check/${transactionId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const applyVoucher = async () => {
    try {
      ...
      const response = await axios.post(
        import.meta.env.VITE_BACKEND_URL + "/api/vouchers/check",
        { code: voucherCode },
        {
          headers: {
            Authorization: `Bearer ${token}`,
