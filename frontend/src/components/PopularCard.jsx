...
function PopularCard() {
  ...
  const fetchProducts = async (category) => {
    ...
    try {
      const url =
        category === "all"
          ? import.meta.env.VITE_BACKEND_URL + "/api/items?take=10&filterBy=popular" // Semua produk
          : import.meta.env.VITE_BACKEND_URL + `/api/items?categoryId=${category}&filterBy=popular&take=10`; // Produk berdasarkan kategori
