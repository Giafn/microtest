...
function NewCard() {
  ...
  const fetchProducts = async (filter) => {
    setLoading(true);
    try {
      const response = await fetch(import.meta.env.VITE_BACKEND_URL + `/api/items?${filter}`);
        ...
        {products.map((product) => (
          <Link to={`/productdetail/${product.id}`} key={product.id} className="block">
            <div className="product-card bg-white border-4 border-[#C62E2E] rounded-lg shadow-lg overflow-hidden relative w-[300px] h-[450px]">
              ...
              <img
                src={product.images && product.images[0] ? import.meta.env.VITE_BACKEND_URL + `/${product.images[0].url}` : "/default-image.png"}
