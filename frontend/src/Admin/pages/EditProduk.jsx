...
const EditProduk = () => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      const token = localStorage.getItem("token");
      try {
        const response = await fetch(import.meta.env.VITE_BACKEND_URL + `/api/items/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        if (data.images && data.images.length > 0) {
          setUploadedImage(import.meta.env.VITE_BACKEND_URL + `/${data.images[0].url}`);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(import.meta.env.VITE_BACKEND_URL + `/api/items/${id}`, formDataObj, {
        headers: {
